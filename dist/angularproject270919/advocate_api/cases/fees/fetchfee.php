...
if($result = mysqli_query($con,$sql))
{
  ...
  {
	  $case_id=$row['case_no'];
	  $sql_rec="select amount from receipt where case_id={$case_id}";
	  $result_rec=mysqli_query($con,$sql_rec);
	  $row_rec=mysqli_fetch_assoc($result_rec);
	  $sql_inv="SELECT AUTO_INCREMENT as inv_id FROM  information_schema.TABLES WHERE TABLE_SCHEMA = 'advocate' AND TABLE_NAME = 'invoice'";
